angular.module("serverApp").controller('ConfCtrl', ['$scope', '$http', '$window', '$timeout', 'NgMap', function ($scope, $http, $window, $timeout, NgMap) {
  ...
  $scope.save = function () {
    ...
    $scope.busy = $http.post('/iop-node/web/rest/api/v1/admin/configuration/save', angular.toJson($scope.configuration))
  ...
  $scope.get = function () {
      $scope.busy = $http.get('/iop-node/web/rest/api/v1/admin/configuration/get', angular.toJson($scope.credentials))
        .then(function successCallback(response) {
