angular.module("serverApp").controller('IdentitiesCtrl', ['$scope', '$http', '$interval', '$filter', '$window', '$location', '$timeout', 'NgMap', function($scope, $http, $interval, $filter, $window, $location, $timeout, NgMap) {
     ...
     var requestActorsType = function() {
             ...
             $scope.busy = $http({
                 method: 'GET',
                 url: '/iop-node/web/rest/api/v1/admin/actors/types'
     ...
     var requestCatalogData = function() {
        ...
        $scope.busy = $http({
            method: 'GET',
            url: '/iop-node/web/rest/api/v1/admin/actors/catalog?actorType='+$scope.selectType+'&offSet='+$scope.offSet+'&max='+$scope.max
        }).then(function successCallback(response) {
     ...
     var requestCheckInData = function() {
         ...
         $scope.busy = $http({
             method: 'GET',
             url: '/iop-node/web/rest/api/v1/admin/actors/check_in?actorType='+$scope.selectType+'&offSet='+$scope.offSet+'&max='+$scope.max
         }).then(function successCallback(response) {
